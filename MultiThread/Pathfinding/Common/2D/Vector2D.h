...
//------------------------------------------------------------------------
enum {clockwise2D = 1, anticlockwise2D = -1};

inline int Vector2D::Sign(const Vector2D& v2)const
{
  if (y*v2.x > x*v2.y)
  { 
    return anticlockwise2D;
  }
  else 
  {
    return clockwise2D;
...
inline Vector2D POINTStoVector2D(const POINTS& p)
{
  return Vector2D(p.x, p.y);
}

inline Vector2D POINTtoVector2D(const POINT& p)
{
